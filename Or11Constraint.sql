...
형식1] 인라인 방식 : 컬럼 생성시 우측에 제약조건을 기술한다.
    ...
    ※ [] 대괄호 부분은 생략 가능하고, 생략시 제약명을 시스템이 자동으로
        부여한다.
*/
...
제약조건 및 테이블목록 확인
    ...
    user_cons_columns : 테이블에 지정된 제약조건명과 컬럼명의 간략한 정보를 저장
    user_constrains : 테이블에 지정된 제약조건의 상세한 정보를 저장
※ 이와 같이 제약조건이나 뷰, 프로시저 등의 정보를 저장하고 있는 시스템 테이블을
    "데이터 사전"이라고 한다.
*/
select * from tab;
...
insert into tb_primary1 (idx, user_id, user_name)
    values (3, 'white', '화이트');
-- PK로 지정된 컬럼에는 null을 입력할 수 없다. 에러가 발생한다.
insert into tb_primary1 (idx, user_id, user_name)
    values ('', 'black', '블랙');
-- 입력된 레코드 확인
SELECT * FROM tb_primary1;
/*
update 문은 정상이지만 idx값이 이미 존재하는 1로 변경했으므로 제약조건 위배로
오류가 발생한다.
*/
...
/*
PK 지정시 my_pk1로 제약명을 지정했으므로 에러 발생시 해당 이름이
콘솔에 출력된다.
*/
...
-- 확인해보면 휴지통에 들어간 것을 볼 수 있다.
SELECT * FROM user_cons_columns;

-- PK는 테이블당 하나만 생성할 수 있다. 따라서 에러가 발생한다.
...
- 테이블 간에 외래키가 설정되어 있다면 자식테이블에 참조값이 존재할 경우
    부모테이블의 레코드는 삭제할 수 없다.
    (ex. 회원제 게시판에서 글을 작성해 놓은 상태라면 그 회원은 삭제할 수 없다.
    참조무경성 에러가 발생된다.)
...
create table tb_foreign1 (
    ...
    f_name varchar2(50),
-- 자식테이블인 tb_foreign1에서 부모테이블인 tb_primary2의 user_id컬럼을 참조하는 외래키를 생성
...
/*
오류발생. 부모테이블에는 gildong이라는 아이디가 없는 상태임.
...
이 경우 반드시 자식테이블의 레코드를 먼저 삭제한 후 부모테이블의
레코드를 삭제해야 한다.
*/
delete from tb_primary2 where user_id='kosmo';
-- 따라서 자식테이블의 레코드를 먼저 삭제한 후 ->
delete from tb_foreign1 where f_id='kosmo';
-- 부모테이블의 레코드를 삭제하면 정상처리 된다.
delete from tb_primary2 where user_id='kosmo';
/*
두 테이블이 외래키(참조키)가 설정되어 있는 경우 부모테이블에
참조할 레코드가 없으면 자식 테이블에 insert 할 수 없다.
...
create table tb_foreign2 (
    ...
    f_name varchar2(30),
    f_date date,
-- tb_foreign2 테이블의 f_id 컬럼이 부모테이블 tb_primary1의 idx컬럼을 참조하는 외래키 생성
...
/*
외래키 삭제시 옵션
[on delete cascade] : 부모 레코드 삭제시 자식레코드까지 같이 삭제된다.
[on delete set null] : 부모 레코드 삭제시 자식레코드 값이 null로 변경된다.
...
/*
부모테이블에서 레코드를 삭제할 경우 on delete cascade 옵션에 의해 자식쪽까지
모든 레코드가 삭제된다. 만약 해당 옵션을 부여하지 않았다면 레코드는 
삭제되지 않고 오류가 발생하게 된다.
*/
...
create table tb_foreign5 (
    f_idx number(10) primary key,
    f_name varchar2(30),
    user_id varchar2(30) constraint tb_foreign5_f5
...
/*
삭제시 부모테이블에서는 레코드가 삭제되지만, 자식테이블에서는 삭제되지 않는다.
...
/*
insert 쿼리에서 컬럼을 명시하지 않으면 null이 입력된다. 따라서 idx 컬럼에는
null을 입력할 수 없으므로 에러가 발생된다.
*/
...
/*
3. pr_dept_const 테이블에 존재하지 않는 부서의 사원이 배정되지 않도록 외래키 
제약조건을 지정하되 제약조건 이름은 pr_emp_dept_fk 로 지정하시오.
*/
