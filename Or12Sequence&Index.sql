...
- 시퀀스는 테이블 생성 후 별도로 만들어야 한다. 따라서 시퀀스는 테이블과
    독립적으로 생성되고 저장한다.
    
형식] create sequnce 시퀀스명
        Increment by N -> 증가치
        Start with N -> 시작값
        Minvalue N -> 시퀀스 최소값
        Maxvalue N -> 시퀀스 최대값
        Cycle 혹은 NoCycle -> 최대값에 도달한 경우 처음부터 다시 시작할지
                                여부 결정.
        Cache 혹은 NoCache -> cache 메모리에 오라클 서버가 시퀀스 값을 할당할지
                                여부를 지정
*/
...
/*
시퀀스 생성 후 최최 실행시에는 currval을 실행할 수 없어 에러가 발생됨.
nextval을 먼저 실행해서 시퀀스를 얻어온 후 실행해햐 한다.
*/
select seq_serial_num.currval from dual;
/*
다음 입력할 시퀀스르 반환한다. 실행할때마다 설정한 증가치만큼 증가된 값이
반환된다.
*/
...
사용할 수 있다. 또한 같은 쿼리문을 여러번 실행하더라도 문제없이 입력된다.
*/
insert into tb_goods values (seq_serial_num.nextval, '크보빵');
SELECT * FROM tb_goods;
/*
단, 시퀀스의 cycle 옵션에 의해 최대값에 도달하면 다시 처음부터 시퀀스가
생성되므로 무결성 제약조건에 위배되어 에러가 발생된다.
...
alter sequence seq_serial_num
    increment by 1
    minvalue 9 -- 최소값은 기존에 설정된 값보다 크면 에러 발생
...
/*
PK로 지정된 컬럼에 일련번호를 입력하는 용도로 주로 사용되므로 최대값, 싸이클,
캐시는 사용하지 않는 것을 권장한다. 
*/
