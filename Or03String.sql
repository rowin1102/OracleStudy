...
파일명 : Or03String.sql
문자열 처리함수
설명 : 문자열에 대해 대소문자를 변환하거나, 문자열의 길이, 자르기 등 문자열을 조작하는 함수
*/

/*
concat(문자열1, 문자열2) : 문자열 1, 2를 서로 연결해서 출력. 문자열 부분은 컬럼명을 사용할 수 있음
...
*/
-- 이와 같이 쿼리하면 결과가 인출되지 않는다. 데이터는 대소문자를 구분하기 때문이다.
SELECT * FROM employees where first_name='john';
-- 입력값의 첫글자만 대문자로 변경 후 쿼리한다.
...
-- 레코드의 입력값 전체를 대문자로 변경 후 조건으로 사용한다.
SELECT * FROM employees where upper(first_name)=upper('john');

/*
lpad(), rpad() : 문자열의 왼쪽, 오른쪽을 특정한 기호로 채울때 사용
형식] lpad('문자열', '전체자리수', '채울문자') => 전체 자리수에서 문자열의 길이만큼을 제외한
    나머지 좌(우)측 부분을 주어진 문자로 채워준다.
*/
...
/*
ltrim(), rtrim() : 좌측, 우측의 '문자' 혹은 '문자열'을 제거할 때 사용.
그리고 공백도 제거할 수 있다.
*/
select
    ltrim(' 좌측공백제거 '), -- 좌측의 공백만 제거됨
    ltrim(' 좌측공백제거 ', '좌측'), -- 좌측에 공백이 포함되어 있어 문자열이 삭제되지 않는다.
    ltrim(trim(' 좌측공백제거 '), '좌측') -- 이 경우 2개의 함수를 중첩해서 사용하여 기능을 수행할 수 있다.
from dual;
...
*/
-- 1.이름의 첫글자만 출력(인덱스 1부터 1글자를 자름)
select first_name, substr(first_name, 1, 1) from employees;
-- 2.이름을 10글자로 간주하여 나머지 부분을 *로 채우기
select rpad(first_name, 10, '*') from employees;
/*
이름의 첫글자를 잘라내고, 이름의 길이(length)를 얻어온다.
이를 통해 *를 첫글자를 제외한 남은 길이만큼을 마스킹 처리할 수 있다.
*/
select first_name, rpad(substr(first_name, 1, 1), length(first_name), '*') from employees;

/*
replace() : 문자열을 다른 문자열로 대체할 때 사용한다. 만약 공백으로 문자열을 대체한다면
    해당 문자열을 삭제하는 결과가 된다.
    형식] replace(컬럼명 or 문자열, '변경 대상의 문자', '변경할 문자')
※trim(), ltrim(), rtrim()의 기능을 replace() 하나로 대체할 수 있으므로 trim()에 비해 replace()가 훨씬 더 사용빈도가 높다.
*/
-- 특정 문자열을 변경
select replace('good morning john', 'morning', 'evening') from dual;
-- 문자열 삭제
select replace('good morning john', 'morning', '') from dual;
