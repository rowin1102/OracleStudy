...
/*
PL/SQL(Procdduarl Language) : 일반 프로그래밍 언어에서 가지고 있는 요소를
    모두 가지고 있으며 DB업무를 처리하기 위해 최적화된 언어
*/
...
'이름'의 경우 2개의 컬럼이 합쳐진 상태이므로 조금 더 넉넉한 크기로 설정해
주는 것이 좋다.
*/
    empName varchar2(50);
...
begin
/*
실행부 : select절에서 가져온 결과를 선언부에서 선언한 변수에 1:1로 대입하여
    값을 지정한다. 이때 into를 사용한다.
*/
    select concat(first_name || ' ', last_name), phone_number
    into
        empName, empPhone
...
/
/*
PL/SQL 문장의 끝에는 반드시 /를 붙여야 하는데, 만약 없으면 호스트환경으로
빠져나오지 못한다. 즉, PL/SQL문장을 탈출하기 위해 필요하다.
...
참조변수 : 특정 테이블의 컬럼을 참조하는 변수로써 동일한 자료형과 크기로
    선언하고 싶을때 사용한다.
형식] 테이블명.컬럼명%type => 테이블에서 '하나'의 컬럼만을 참조한다.
*/
/*
시나리오] 부서번호 10인 사원의 사원번호, 급여, 부서번호를 가져와서 아래 변수에
대입후 화면상에 출력하는 PL/SQL문을 작성하시오. 
...
declare
/*
사원테이블의 전체 컬럼을 참조하는 참조변수로 선언. 테이블명 뒤에 
%rowtype을 붙여주면 된다.
*/
    emp_row employees%rowtype;
begin
-- 와일드카드 *를 통해 얻어온 전체컬럼을 변수 emp_row에 한꺼번에 저장할 수 있다.
    select *
    into emp_row
    from employees
    where employee_id=100;
/*
row_row에는 전체컬럼의 정보가 저장되므로 출력시 변수명.컬럼명 형태로 
기술해야 한다.    
*/
...
-- 예제5] 복합변수
/*
복합변수 : class를 정의하듯 필요한 자료형을 하나로 묶어 새로운
    자료형을 만든 후 생성하는 변수를 말한다.
형식]
...
-- 3개의 값을 저장할 수 있는 복합변수 자료형을 선언한다.
    ...
    emp_id employees.employee_id%type, -- 사원테이블을 참조하는 참조변수로 선언
    emp_name varchar2(50), -- 일반변수로 선언
    emp_job employees.job_id%type
    );
/*
앞에서 정의한 자료형을 통해 복합변수를 선언한다. 이 변수는 3개의 값을
저장할 수 있다.
*/
    record3 emp_3type;
begin
...
declare
/*
인출시 데이터의 변화가 없는 3개의 컬럼은 기존 테이블을 참조해서 생성하고,
이름은 2개의 컬럼을 합쳐야 하므로 넉넉한 크기로 생성한다.
*/
...
-- 예제6] 바인드변수
/*
바인드변수 : 호스트환경에서 선언된 변수로써 비 PL/SQL 변수이다.
    호스트 환경은 SQL> 프롬프트가 있는 상태로 명령어(쿼리문)를 입력할 수
    있는 상태를 말한다.
형식] var 혹은 varuable을 사용해서 생성
    var 변수명 자료형(크기);
*/
-- 호스트 환경에서 바인드 변수 선언
...
CMD에서는 개별적으로 샐행해도 문제가 없지만, 디벨로퍼에서는
바인드 변수로부터 마지막 print까지 묶은 후 실행해야 결과가 제대로
출력된다.
*/
