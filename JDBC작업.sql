-- Java에서 JDBC 연동하기 -> education 계정

-- member 테이블 생성
create table member (
    /* 
    아이디, 비번, 이름은 문자타입으로 정의. not null로 빈값을
    허용하지 않는 컬럼으로 정의
    */
    ...
    */
    ...
    primary key (id) -- 아웃라인 방식으로 아이디 컬럼을 PK로 지정
);
...
Java와 같은 외부 프로그램을 통해 입력하는 데이터는 자동으로 commoit이 되므로
별도의 처리를 하지 않아도 된다.
*/
...
(
    ...
    id varchar2(30) not null, -- 회원제이므로 회원아이디
...
*/
alter table board -- 외래키는 자식테입르에서 설정함
    add constraint board_mem_fk foreign key (id) -- 제약명 지정
        references member (id); -- 부모테이블에 참조할 컬럼

-- 자식테이블인 board에 더미데이터 추가
/*
아이디 aaaa는 부모테이블인 member에 존재하지 않으므로 insert문 실행시
에러가 발생한다.
*/
insert into board (num, title, cnotent, id, postdate, visitcount)
...
/*
2개의 테이블이 참조관계에 있다면 데이터 입력시 부모테이블부터 입력해야 한다.
...
is 
    retStr varchar2(50); -- 마스킹 후 반환할 문자열을 저장할 변수 선언
begin
...
-- Java에서 입력한 값을 받을 인파라미터 정의
    ...
    p_pass in varchar2,
    p_name in varchar2,
    returnVal out number -- 입력성공여부를 반환할 숫자형식의 아웃파라미터
)
is -- 변수 선언 없음
begin
...
-- 행의 변화가 생기므로 반드시 커밋을 해야한다.
        commit;
    else
-- 실패한 경우에는 0을 반환
        returnVal := 0;
    end if;
-- 프로시저는 별도의 return없이 아웃파라미터에 값을 할당하기만 하면 된다.
end;
/
...
execute MyMemberInsert('pro02', '1222', '프로시저2', :i_result);
-- 결과 확인
print i_result;
...
create or replace procedure MyMenberDelete (
    member_id in varchar2, -- 인파라미터 : 삭제할 아이디
    returnVal out varchar2 -- 아웃파라미터 : 삭제 결과
)
is
begin
...
-- 존재하지 않는 아이디로 테스트
execute MyMenberDelete('test99', :delete_var);
-- 존재하는 아이디로 테스트
execute MyMenberDelete('pro01', :delete_var);
...
-- 인파라미터로 전달된 비번과 DB에서 가져온 비번 비교
        if member_pw=user_pass then
            returnVal := 2; -- 둘 다 일치하는 경우 2로 설정
        else
            returnVal := 1; -- 아이디만 일치하는 경우 1로 설정(비번 틀림)
        end if;
    else
        returnVal := 0; -- 회원정보가 틀린 경우 0으로 설정
    end if;
end;
/
...
-- 둘 다 일치하는 경우(인증성공)
execute MyMemberAuth('spring', '1234', :member_auth);
print member_auth;
-- 아이디만 일치하는 경우
execute MyMemberAuth('spring', '1234암호틀림', :member_auth);
print member_auth;
-- 회원정보가 없는 경우
