...
파일명 : Or09Join.sql
테이블조인
설명 : 2개 이상의 테이블을 동시에 참조하여 데이터를 인출해야 할때 사용하는 SQL문
*/
...
검색할때 사용한다.
- 일반적으로 가장 기본키(Primary Key)와 외래키(Foreign Key)를 사용하여 join하는
경우가 대부분이다.
- 2개의 테이블에 동일한 이름의 컬럼이 존재해야 하고, 이 경우 '테이블명.컬럼명'과
같이 기술한다.
...
select 
    employee_id, first_name, last_name, email, employees.department_id, department_name
    from employees inner join departments
        on employees.department_id = departments.department_id;
/*
실행결과에서는 소속된 부서가 없는 1명을 제외한 나머지 106명의 레코드가 인출된다.
즉, inner join은 조인한 테이블 양쪽에 만족되는 레코드를 대상으로 인출하게 된다.
*/
...
-- 5. join 쿼리문 작성
/*
양쪽 테이블에 동시에 존재하는 컬럼인 경우에는 반드시 테이블명이나 별칭을
명시해야 한다.
*/
select
...
형식2] 오라클 방식
    ...
    where
        ...
        and 조건1 or 조건2..;
표준방식에서 사용한 inner join과 on을 제거하고 조인의 조건을 where절에
표기하는 방식이다.
*/

/*
시나리오] 사원테이블과 부서테이블을 조인하여 각 직원이 어떤부서에서
    근무하는지 출력하시오. 단, 오라클으로 작성하시오.
    출력결과 : 사원아이디, 이름1, 이름2, 이메일, 부서번호, 부서명
*/
select 
...
/*
2] outer join(외부조인)
outer join은 inner join과는 달리 두 테이블간의 조인조건이 정확히 일치하지 않아도
기준이 되는 테이블에서 레코드를 인출하는 방식이다.
outer join을 사용할때는 반드시 기준이 되는 테이블을 결정하고 쿼리문을 작성해야한다.
...
*/
select
    employee_id, first_name, last_name, Em.department_id, department_name, city
from employees Em
...
부서가 지정되지 않은 사원까지 인출되기 때문인데, 이 경우 부서쪽에 레코드가
없으므로 null이 출력된다.
*/
...
- 위의 경우 왼쪽 테이블이 변경된다.
- 기준이 되는 테이블을 변경하고 싶다면 테이블의 위치를 옮겨준다. (+)를 옮기지 않는다.
*/
...
select first_name, hire_date, to_char(hire_date, 'yy') from employees;
-- 2007년에 입사한 사원을 인출(19개)
select first_name, hire_date from employees 
    where to_char(hire_date, 'yyyy') = '2007';
/*
외부조인을 표준방식으로 작성한 후 결과를 확인한다. nvl() 함수를 통해 null값을
지정한 값으로 변경해준다. 결과는 모두 19개가 인출된다.
*/
select 
    E.employee_id, first_name, last_name, nvl(department_name, '<부서없음>')
from employees E left outer join departments D
    on E.department_id = D.department_id
where to_char(hire_date, 'yyyy') = '2007';

-- 시나리오] 위 커리문을 오라클방식으로 변경하시오.
select 
    E.employee_id, first_name, last_name, nvl(department_name, '<부서없음>') "department_name"
from employees E, departments D
where E.department_id = D.department_id (+) and
    to_char(hire_date, 'yyyy') = '2007';
/*
오라클 방식으로 변경시 outer join 연산자인 (+)를 생략하면 inner join이 되므로
작성에 주의해야 한다.
*/
...
즉, 자신의 테이블과 조인을 맺는 것이다. 셀프조인에서는 별칭이 테이블을 구분하는
구분자의 역할을 하므로 매우 중요하다.
...
select
    empClerk.employee_id, empClerk.first_name,
    empClerk.last_name, 
    empManager.employee_id, empManager.first_name,
...
using절에 사용된 컬럼은 Join이 되는 테이블 양쪽에 동시에 존재하는 컬럼인 것을
전재로 작성되기 때문에 굳이 별칭을 사용할 필요가 없기 때문이다.
즉, using은 테이블의 별칭 및 on절을 제거하여 보다 심플하게 join 쿼리문을 작성할
수 있게 해준다.
*/
-- 18개 레코드 인출됨
select
...
-- 문제에서 주어진 지역정보로 지역 일련 번호를 확인(결과 : 1500)
...
/*
2005년에 입사했고, 50번 부서(Shipping)에 근무하는 사원의 정보를 인출하는 inner join
쿼리문을 작성한다.
*/

select
    employee_id, first_name, last_name, to_char(salary, '$9,000') salary,
...
from employees
    ...
    inner join countries using(country_id)
where
    state_province='California' and city='South San Francisco' and
