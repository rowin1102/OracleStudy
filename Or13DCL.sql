...
사용자 권한 관리
설명 : 새로운 사용자 계정을 생성하고 시스템 권한 부여 및 회수하는 방법을 학습
*/

/*
사용자 계정 생성 및 권한 설정 : 해당 작업은 DBA 권한이 있는 
    최고관리자(sys, system)으로 접속한 후 실행해야 한다.
...
/*
create table 권한 부여 후 테입르 생성 및 desc 명령으로 생성된 테이블의
스키마를 확인할 수 있다.
*/
...
/*
exit 혹은 quit 명령으로 접속을 해제한 후 다시 접속하면 기존 암호로는
접속할 수 없다. 변경한 암호로 접속해야 한다.
*/
...
/*
아래 2개의 Role은 오라클에서 기본적으로 제공된다. 접속 및 테이블 생성등의
대부분의 기본작업을 할 수 있는 권한이 포함되어 있다.
*/
grant connect, resource to test_user2;

/*
4-1] Role 생성하기 : 사용자가 원하는 권한을 묶어서 새로운 롤 생성
*/
create role my_role;
...
-- 우리가 직접 생성한 롤을 통해 권한을 부여한다.
grant my_role to test_user3;
/*
my_role을 통해 권한을 부여했으므로 접속 및 테입르 생성이 정상적으로
실행된다.
*/
...
drop role my_role;
/*
test_user3은 my_role을 통해 권한을 부여받았으므로 해당 롤을 삭제하면
모든 권한이 회수(Revoke)된다.
...
/*
접속권한 회수 후 접속을 시도할때 비밀번호가 틀리면 '부적합' 에러가 발생되고,
