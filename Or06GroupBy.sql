...
파일명 : OR06GroupBy.sql
그룹함수(select문 2번째)
설명 : 전체 레코드에서 통계적인 결과를 구하기 위해 하나 이상의 레코드를 그룹으로
    묶어서 연산한 후 결과를 반환하는 함수 혹은 쿼리문 학습
*/
-- 사원테이블에서 담당업무 인출. 별도의 조건없으므로 107개가 인출됨.
select job_id from employees;
/*
distint : 동일한 값이 있는 경우 중복된 레코드를 제거한후 하나의 레코드만 가져와서 보여준다.
    순수한 하나의 레코드이므로 통계적인 값을 계산할 수 없다.
*/
select distinct job_id from employees;
/*
    group by : 동일한 값이 있는 레코드를 하나의 그룹으로 묶어서 인출한다.
        보여지는 것은 하나의 레코드이지만 다수의 레코드가 하나의 그룹으로 묶여진
...
-- count() 함수를 통해 인출된 행의 갯수는 아래와 같이 일반적인 select문으로 검증할 수 있다.
...
/*
count() 함수를 사용할때는 위 2가지 방법 모두 가능하지만 * 사용을 권장한다.
    컬럼의 특성 혹은 데이터에 따른 방해를 받지 않으므로 실행속도가 빠르다.

count() 함수의 사용법
    1. count(all 컬럼명) : 디폴트 사용법으로 컬럼명 전체의 레코드를 기준으로 카운트
    2. count(distinct 컬럼명) : 중복을 제거한 상태에서 카운트
*/
select
    count(job_id),
    count(all job_id),
...
-- 전체사원 중 가장 낮은 급여는 얼마인가요? 즉, 급여의 최소값이 얼마인가요?
...
/*
group by절 : 여러개의 레코드를 하나의 그룹으로 묶은 후 그룹화하여 결과를 반환하는 쿼리문
...
/*
아래 쿼리문은 부서번호를 그룹으로 묶어서 결과를 인출하므로, 이름을 기술하면 에러가 발생된다,
...
/*
퀴즈] 사원테이블에서 각 부서별 사원수와 평균급여가 얼마인지 출력하는 쿼리문을 작성하시오. 
출력결과 : 부서번호, 급여총합, 사원총합, 평균급여
출력시 부서번호를 기준으로 오름차순 정렬하시오. 
*/
select 
    department_id "부서번호", trim(to_char(sum(salary), '999,000')) "급여총합", count(*) "사원총합", 
    trim(to_char(avg(salary), '999,000')) "평균급여"
    from employees group by department_id order by department_id;

/*
having : 물리적으로 존재하는 컬럼이 아닌 그룹함수를 통해 논리적으로 생성된 컬럼의 조건을
    추가할 때 사용한다. 해당 조건을 where절에 추가하면 에러가 발생한다.
*/
/*
...
*/
/*
같은 부서에서 근무하더라도 담당업무는 다를 수 있으므로 이 문제에서는 group by 절에
2개의 컬럼을 명시해야한다. 즉, 부서로 그룹화 한 후 다시 담당업무별로 그룹화한다.
*/
select
    department_id, job_id, count(*), avg(salary)
    from employees 
    where count(*)>10 -- 이 부분에서 에러 발생
    group by department_id, job_id;
/*
담당업무별 사원수는 물리적으로 존재하는 컬럼이 아니므로 where절에 추가하면 에러가 발생한다.
이 경우에는 having절에 조건을 추가해야 한다.
Ex) 급여가 3000인 사원 => 물리적으로 존재하므로 where절에 추가
    평균급여가 3000인 사원 => 상황에 맞게 논리적으로 만들어낸 결과이므로 having절에
        추가해야 한다.
*/
