...
SELECT * FROM kosmo;
/*
Dual테이블 : 하나의 행으로 결과를 출력하기 위해 제공되는 테이블로 오라클에서 자동으로 생성되는 논리적 테이블이다.
...
select trunc(12345.12345) from dual; -- 12345 출력
select trunc(12345.12345, -2) from dual; -- 12300 출력
...
-- 또는 영업사원은 커미션이 있으므로 commission_pct is not null로도 인출할 수 있다.

-- 2.커미션을 계산해서 이름과 함께 출력한다.
SELECT first_name, salary, commission_pct, salary+(salary*commission_pct) FROM employees where job_id like 'SA_%';
